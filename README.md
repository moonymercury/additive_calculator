...
### 可能可以繼續增進的方向:
- [x]方向 A：擴充數據集（解決 PCI 權重過低問題）
目前只有 57 筆，AI 很容易產生偏差。如果能擴充到 1000 筆，AI 就能接觸到更多「長得像但類別不同」的分子，PCI 的權重會顯著提升。

方向 B：啟動「模組四」的分子對接 (Molecular Docking)，這是解決「AI 覺得安全 vs. 傳統覺得危險」的最佳裁判。
...
方向F: 隨機組合模擬：利用你的資料庫，隨機生成 10,000 種「虛擬產品成分表」（例如：3 種抗氧化劑 + 2 種乳化劑）。

方向G: 計算 PCI/BDS/CIB 分數：作為模型的初步標籤 (Initial Label)。

方向H: 訓練深度學習模型：輸入成分列表，直接預測該組合的「等效劑量風險」。
